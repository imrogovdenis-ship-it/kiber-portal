...
test('homepage titles are explicit nominative labels for all launch robots',()=>{
 ...
 assert.equal(names['arenda-robota-sofiya'],'Робот София');assert.equal(names['arenda-robota-tron'],'Модульный робот Tron');assert.equal(names['arenda-robota-hudozhnika-a4'],'Робот-художник A4');
