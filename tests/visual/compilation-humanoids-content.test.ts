...
test('seven explicit models, approved local gallery and truthful prices',()=>{
 ...
 assert.equal(p.media.length,7);assert.ok(p.media.every(m=>m.actualDescription && m.seoAlt && m.src.startsWith('/images/kiber-94-preview/')));
...
test('preview contains researched content, two quotes and no placeholders',()=>{
 ...
 assert.doesNotMatch(html,/static\.tildacdn\.com|визуальной системе PR8|Блок усиливает|data-block-id="video"/);
 assert.ok(html.includes('https://www.kiber-portal.ru/roboty-gumanoidy/'));
 assert.ok(html.includes('ItemList'));assert.ok(html.includes('noindex'));
 const ids=[...html.matchAll(/\sid="([^"]+)"/g)].map(m=>m[1]);assert.equal(ids.length,new Set(ids).size);
 assert.equal(p.blocks.scenarios.items.length,6);assert.equal(p.blocks.faq.items.length,6);
});
