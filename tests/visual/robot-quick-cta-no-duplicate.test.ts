...
function title(name:string,robotTypeAccusative='робота-официанта'){const statements=source.split('\n').filter(l=>/^const quickCta(Name|Title) =/.test(l)).join('\n');return runInNewContext(statements+';quickCtaTitle',{robotSlug:'arenda-bellabot',robotTypeAccusative,template:{robot:{name,priceDisplay:'от 9 500 ₽ / час'}}});}
...
test('pricing CTA keeps an already inflected robot name with a different category label',()=>{assert.equal(title('робота Клипмейкер','робота для съёмки'),'Арендуйте робота Клипмейкер для мероприятия от 9 500 ₽ / час');});
