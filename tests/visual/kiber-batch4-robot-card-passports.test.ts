...
test('KIBER Batch 4 robot-card SEO passports are exact-model scoped and review-gated', () => {
  const summary = readJson('data/seo/robot-card-passports/batch4-robot-cards.robot-card-seo-passports-summary.json');
...
test('KIBER Batch 4 robot-card galleries use converted source-gallery assets only', () => {
  const manifest = readJson('data/review/batch4-robot-card-gallery-assets.json');
  ...
  for (const slug of slugs) {
    ...
    assert.ok(robot.assets.every((asset: { src: string }) => asset.src.startsWith('/images/kiber-94-preview/batch4-robot-cards/')), `${slug} uses batch3 preview gallery assets`);
...
test('KIBER Batch 4 rendered preview keeps approved robot-card CTA, media and Gosha gates when build exists', () => {
  ...
  if (!firstHtml.includes('/images/kiber-94-preview/batch4-robot-cards/')) return;
  for (const slug of slugs) {
