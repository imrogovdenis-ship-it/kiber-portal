...
test('KIBER Batch 3 robot-card SEO passports are exact-model scoped and review-gated', () => {
  const summary = readJson('data/seo/robot-card-passports/batch3-robot-cards.robot-card-seo-passports-summary.json');
...
test('KIBER Batch 3 robot-card galleries use converted source-gallery assets only', () => {
  const manifest = readJson('data/review/batch3-robot-card-gallery-assets.json');
  ...
  for (const slug of slugs) {
    ...
    assert.ok(robot.assets.every((asset: { src: string }) => asset.src.startsWith('/images/kiber-94-preview/batch3-robot-cards/')), `${slug} uses batch3 preview gallery assets`);
...
test('KIBER Batch 3 rendered preview keeps approved robot-card CTA, media and Gosha gates when build exists', () => {
  ...
  if (!firstHtml.includes('/images/kiber-94-preview/batch3-robot-cards/')) return;
  for (const slug of slugs) {
    ...
    assert.doesNotMatch(html, /специальная иллюстрация блока/i, `${slug} capability alts must be factual, not boilerplate SEO text`);

    const capabilities = html.match(/<section[^>]+data-block-id="capabilities"[\s\S]*?<\/section>/)?.[0] ?? '';
    const scenarios = html.match(/<section[^>]+data-block-id="scenarios"[\s\S]*?<\/section>/)?.[0] ?? '';
    const faq = html.match(/<section[^>]+data-block-id="faq"[\s\S]*?<\/section>/)?.[0] ?? '';
    assert.equal((capabilities.match(/<article/g) ?? []).length, 6, `${slug} renders 6 capability cards`);
    assert.equal((scenarios.match(/<article/g) ?? []).length, 6, `${slug} renders 6 scenario cards`);
    assert.equal((faq.match(/<details/g) ?? []).length, 8, `${slug} renders 8 FAQ items`);

    const visibleText = html.replace(/<script[\s\S]*?<\/script>/g, ' ').replace(/<style[\s\S]*?<\/style>/g, ' ').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
    const forbiddenPublicCopy = [
      'робот/интерактивный формат',
      'Карточка объясняет',
      'лучше работает, когда у формата есть конкретная роль',
      'Сценарий под мероприятие',
      'роботические сюрпризы',
      'KIBER-94 preview',
      'роботов для событий',
      'Не просто робот, а готовый event-сервис',
      'произведёт «вау-эффект»',
      'Необычный подарок директору',
      'Unitree G1 или Agibot X2',
      'Необычное поздравление на свадьбе',
      'велком-зона на свадьбе',
    ];
    for (const phrase of forbiddenPublicCopy) {
      assert.doesNotMatch(visibleText, new RegExp(phrase, 'i'), `${slug} must not render generic/internal phrase: ${phrase}`);
    }
    assert.doesNotMatch(visibleText, /большого формата/i, `${slug} must not leak dirty A4 source naming into visible copy`);
    const articles = html.match(/<section[^>]+id="articles"[\s\S]*?<\/section>/)?.[0] ?? '';
    assert.doesNotMatch(articles.replace(/<[^>]+>/g, ' '), /роботов для событий|Необычный подарок директору|Unitree G1 или Agibot X2|Необычное поздравление на свадьбе|велком-зона на свадьбе/i, `${slug} related articles must stay relevant`);
    if (slug === 'arenda-robota-hudozhnika-a4') {
      assert.doesNotMatch(html, />большого формата</i);
