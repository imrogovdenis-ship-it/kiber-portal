...
test('KIBER-49 exposes media Git hygiene as a CI gate', () => {
  ...
  assert.match(script, /git-lfs is not installed|gitLfsAvailable/);
...
test('KIBER-49 prevents new ordinary-Git originals and keeps runtime media optimized', () => {
  ...
  assert.equal(report.status, 'passed_with_storage_migration_blocker');
  ...
  assert.ok(report.summary.trackedReviewOriginals > 0, 'legacy review originals should be explicitly inventoried until storage migration');
  assert.ok(report.blockers.some((item: string) => item.includes('Git LFS') || item.includes('object storage')));
