...
class Contracts(unittest.TestCase):
 def test_owner_order_and_reuse(self):
  ...
  def kind(x):
   ...
   if x.get('id')=='catalog':return 'product'
   if x.get('id')=='catalog-repeat':return 'catalog'
  ...
  self.assertEqual(len(p.select('.article-blocks__featured-model')),3);self.assertEqual(len(p.select('#catalog-repeat .robot-card')),3)
