## Текущий canonical-кандидат (не публикация)

Обновление подготовки: 24 робота, 6 статей и 1 подборка собраны на публичных маршрутах. 251/251 source tests; полный локальный CI PASS. Раздельные бюджеты начальной/полной загрузки утверждены владельцем и проверены в Chromium. Старые этапные сведения ниже — исторический контекст, актуальное состояние в `docs/review/canonical-release/RELEASE_STATUS.json`. Production остаётся NO-GO; отдельные runtime/merge/deploy разрешения не получены.
