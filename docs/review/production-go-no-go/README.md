...
Обновление подготовки: 24 робота, 6 статей и 1 подборка собраны на публичных маршрутах. 251/251 source tests; 35/36 post-build gates. Текущий блокер CI — старый суммарный бюджет изображений; пределы не ослаблены. Старые этапные сведения ниже — исторический контекст, актуальное состояние в `docs/review/canonical-release/RELEASE_STATUS.json`. Production остаётся NO-GO; отдельные runtime/merge/deploy разрешения не получены.
