...
const script=readFileSync('infra/jino-preview/manual-lead-check/contact-lead-form-popup.js','utf8');
...
try {
 for (const scenario of ['bad-json','ok-false','preview','success','preview-host']) {
  ...
  await page.route('**/*', async route=>{
   ...
   if(req.method()==='POST'){
    assert.ok(scenario!=='preview-host'||req.url().endsWith('/manual-lead-check.php'));posts++;await new Promise(r=>setTimeout(r,100));
  ...
  if(scenario==='success'||scenario==='preview-host'){
  ...
  assert.equal(posts,1,'one intercepted POST only');
