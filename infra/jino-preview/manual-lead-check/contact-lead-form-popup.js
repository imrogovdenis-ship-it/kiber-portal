(() => {
  ...
  const manualPreview = location.hostname === 'jino-preview.kiber-portal.ru';
  ...
  const failureMessage = manualPreview ? 'Проверка не завершена или уже закрыта. Сообщите об этом в чате; повторно отправлять не нужно.' : 'Не получилось отправить заявку. Попробуйте написать нам в мессенджер.';
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    if (!manualPreview && (form.dataset.leadFormLive !== 'true' || !['www.kiber-portal.ru', 'kiber-portal.ru'].includes(window.location.hostname))) {
    ...
    try {
      ...
      if (manualPreview) body.set('manual_check', '1');
      const response = await fetch(manualPreview ? '/manual-lead-check.php' : form.action, {
        method: 'POST',
        body,
