...
try {
 for(const width of [320,360,390,430,768,1440]){
  const context=await browser.newContext({viewport:{width,height:width===320?568:900}});await context.route('**/*',route=>route.request().url().startsWith(origin+'/')?route.continue():route.abort());const page=await context.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));const trackers=[];page.on('request',r=>{if(/mc\.yandex|metrika|google-analytics|umami|top\.mail\.ru|vk\.com\/rtrg/.test(r.url()))trackers.push(r.url());});
  ...
  await page.goto(origin+'/cookie-policy/',{waitUntil:'networkidle'});await page.locator('[data-analytics-toggle]').click();await page.locator('input[value=extended]').check();await page.locator('#analytics-settings button[type=submit]').click();await page.reload({waitUntil:'networkidle'});assert.equal(await page.getAttribute('html','data-cookie-consent'),'accepted');assert(!(await page.locator('#cookie-consent').isVisible()));
  await page.locator('[data-analytics-toggle]').click();await page.locator('input[value=basic]').check();await page.locator('#analytics-settings button[type=submit]').click();assert.equal(await page.getAttribute('html','data-cookie-consent'),'rejected');
  await page.locator('[data-analytics-toggle]').click();await page.locator('input[value=off]').check();await page.locator('#analytics-settings button[type=submit]').click();await page.reload({waitUntil:'networkidle'});assert.equal(await page.getAttribute('html','data-analytics-mode'),'off');assert(!(await page.locator('#cookie-consent').isVisible()));assert.equal(await page.locator('footer [data-cookie-settings]').count(),0);
  const bottom=await page.evaluate(()=>document.querySelector('#analytics-settings').getBoundingClientRect().top>=document.querySelector('.legal-page__body').getBoundingClientRect().bottom);assert(bottom,'settings follow policy body');
