...
try{for(const slug of ['arenda-unitree-g1','arenda-agibot-x2'])for(const width of [390,768,1440]){
 ...
 const data=await page.evaluate(()=>({overflow:document.documentElement.scrollWidth>innerWidth,broken:[...document.images].filter(i=>i.getClientRects().length&&(!i.complete||!i.naturalWidth)).map(i=>i.getAttribute('src')),caps:[...document.querySelectorAll('[data-block-id="capabilities"] img')].map(i=>({width:i.getBoundingClientRect().width,columnWidth:i.closest("article").getBoundingClientRect().width,ratio:i.getBoundingClientRect().width/i.getBoundingClientRect().height,natural:i.naturalWidth/i.naturalHeight,bg:getComputedStyle(i).backgroundColor,height:i.getBoundingClientRect().height,fit:getComputedStyle(i).objectFit,alt:i.alt,src:i.getAttribute('src')})),related:[...document.querySelectorAll('[data-block-id="relatedCatalog"] a')].map(a=>a.getAttribute('href'))}));
 assert(!data.overflow);assert.equal(data.broken.length,0);assert.equal(data.caps.length,6);for(const x of data.caps){assert(Math.abs(x.width-x.columnWidth)<1,'Image must fill column');assert(Math.abs(x.ratio-16/9)<.01);assert.equal(x.bg,'rgba(0, 0, 0, 0)');assert.equal(x.fit,'cover');assert(!x.alt.includes('для блока'));}assert(!data.related.some(x=>x.includes(slug)));assert.equal(errors.length,0);
