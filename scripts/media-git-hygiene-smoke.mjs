...
const requiredGitAttributes = ['site-export/images/** filter=lfs diff=lfs merge=lfs -text'];
...
const gitattributes = existsSync(resolve(root, '.gitattributes')) ? read('.gitattributes') : '';
for (const token of requiredGitAttributes) {
  if (!hasToken(gitattributes, token)) failures.push(`.gitattributes must route ${token} through Git LFS`);
}
...
try {
  ...
  lfsTrackedFiles = execFileSync('git', ['lfs', 'ls-files', '--name-only'], { cwd: root, encoding: 'utf8' }).split('\n').filter(Boolean);
} catch {
  gitLfsAvailable = false;
  warnings.push('git-lfs is not installed on this host; KIBER-49 variant A requires Git LFS before migration can be closed.');
}
const lfsTrackedSet = new Set(lfsTrackedFiles);
const reviewOriginalsNotInLfs = trackedOriginals.filter((item) => !lfsTrackedSet.has(item.path));
if (reviewOriginalsNotInLfs.length) {
  failures.push(`${reviewOriginalsNotInLfs.length} review originals under site-export/images are not tracked by Git LFS`);
}
...
const lfsTrackedOriginals = trackedOriginals.filter((item) => lfsTrackedSet.has(item.path));
const report = {
  issue: 'KIBER-49',
  status: failures.length ? 'failed' : 'passed_lfs_migrated',
  ...
  policy: {
    ...
    ordinaryGitOriginalPolicy: 'KIBER-49 variant A: legacy site-export/images originals are tracked through Git LFS pointers; new originals/raw assets are ignored unless explicitly migrated to LFS',
  ...
  gitLfsAvailable,
  gitLfsTrackedFiles: lfsTrackedFiles.length,
  summary: {
    ...
    lfsTrackedReviewOriginals: lfsTrackedOriginals.length,
    reviewOriginalsNotInLfs: reviewOriginalsNotInLfs.length,
...
console.log(`KIBER-49 media Git hygiene smoke passed: ${publicImages.length} runtime images / ${(publicTotalBytes / 1024).toFixed(1)} KiB; ${lfsTrackedOriginals.length}/${trackedOriginals.length} review originals tracked through Git LFS.`);
