...
const root=resolve('dist');const requests=[];const responses=[];const errors=[];const providerSettings=[];
await ctx.route('https://www.kiber-portal.ru/**',async route=>{
 ...
 return route.fulfill({status:200,contentType:({'.html':'text/html','.js':'application/javascript','.css':'text/css','.svg':'image/svg+xml','.json':'application/json','.webp':'image/webp','.woff2':'font/woff2'})[extname(p)]||'application/octet-stream',body:readFileSync(p)});
});
...
try{
 ...
 const after=requests.length;await page.waitForTimeout(2000);assert.equal(requests.length,after);assert.equal(await page.getAttribute('html','data-cookie-consent'),'rejected');
 ...
 console.log(JSON.stringify({status:'PASS',real,checks:['new consent','old consent invalidated','reject','single init','masked form DOM','revocation reload','ym cookies removed','lead route excluded'],requests,responses,providerSettings,errors}));
